...
| Reference | The reference the pro named at the background step has read back the pro's **own SAAHAA code** | `bgReference` (on / off) |
...
The code the reference reads back is the pro's **own SAAHAA code**, which the
pro passes to them. Nothing is texted, and nothing ever will be — that is the
product's design, not a sandbox waiting on a rail.

The hole this leaves is unchanged and worth stating plainly: a pro who wants to
lie to this step can, because they know their own code. It was equally true when
the code was four random digits shown on the pro's screen. The other five lines
do not have that hole — jobs, ratings, disputes and vouches are things *other
people* did on the record. If it bothers you, turn `bgReference` off and raise
`bgVouches` instead; that is what the dial is for. What would actually close it
is the owner ringing the reference, which is a phone call, not a rail.
